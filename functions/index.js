...
async function testGetAirmen(db) {
  let snapshot = await db.collection('airmen').get();
...
//  let snapshot = await db.collection('airmen').get().then(out => {return {result: true, data: out}});
//  let output = [];
//
//  if (snapshot.result) {
